                ...
                <div class="grid lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <span class="text-gray-600">{{ count($cartItems) }}
                            ...
                            <div class="bg-white rounded-lg shadow-md p-6 cart-item" data-id="{{ $item['id'] }}">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        <button
                                            class="w-8 h-8 flex items-center justify-center border rounded-full hover:bg-gray-100 transition-colors quantity-btn"
                                    ...
                                    <div class="text-right">
                                        <div class="text-lg font-bold text-pink-600">Rs. {{ number_format($item['subtotal']) }}
                                        </div>
...
@push('scripts')
    <script>
        ...
        function showError(message) {
            ...
            const existingErrors = document.querySelectorAll('.cart-error-notification');
            existingErrors.forEach(error => error.remove());

            const errorDiv = document.createElement('div');
            errorDiv.className = 'cart-error-notification fixed top-4 right-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded z-50';
            errorDiv.innerHTML =
                '<div style="display: flex; align-items: center; justify-content: space-between;">' +
                '<span style="flex: 1; margin-right: 10px;">' + message + '</span>' +
                '<button onclick="this.parentElement.parentElement.remove()" style="background: none; border: none; color: #dc2626; font-size: 18px; cursor: pointer; line-height: 1; padding: 0; margin-left: 10px;">' +
                '<i class="fas fa-times"></i>' +
                '</button>' +
                '</div>';
